...
DEFAULT_PACKAGE_URL="https://haoxingjun-test.tos-cn-beijing.volces.com/openclaw-session-search/session-search-0.2.4.tgz"
...
if ! pwd >/dev/null 2>&1; then
  ...
  cd "${OPENCLAW_STATE_DIR_RESOLVED:-/tmp}" 2>/dev/null || cd /tmp
fi
...
verify_install() {
  ...
  if timeout 20s openclaw gateway call session-search.search \
    ...
    log "session-search gateway probe skipped or failed; plugin may be disabled"
    sed -n '1,120p' /tmp/openclaw-session-search-probe.err >&2
    log "to enable later, run: openclaw plugins enable $PLUGIN_ID && openclaw gateway restart"
  fi
}
...
main() {
  ...
  log "done"
  log "installed plugin without changing enablement state"
  log "if enabled, Feishu commands are available: /session-search <keyword>"
}
